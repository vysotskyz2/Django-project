...
class BuyerStatisticsService:
    ...
    def get_statistics(self, buyer_id: int, requesting_user: User) -> dict:
        try:
            ...
            raise NotFound(detail="Buyer not found.") from None
        ...
        for offer in purchases_qs:
            unit_price = (
                offer.offered_price.amount if offer.offered_price is not None else Decimal("0.00")
            )
            ...
            purchases.append(
                {
                    ...
                    "dealership_name": (offer.dealership.name if offer.dealership_id else None),

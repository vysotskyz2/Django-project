...
class SupplierInventoryRepository:
    def get_available_for_car(self, car, min_quantity: int) -> QuerySet[SupplierInventory]:
        return SupplierInventory.objects.select_related("supplier", "car").filter(
            car=car,
    ...
    def get_all_cars_with_stock(self):
        car_ids = (
            SupplierInventory.objects.filter(quantity__gte=1, supplier__is_deleted=False)
    ...
    def lock_for_update(self, supplier: Supplier, car) -> SupplierInventory:
        return SupplierInventory.objects.select_for_update().get(supplier=supplier, car=car)
...
class SupplierPromotionRepository:
    def get_active_for(
        ...
        return SupplierPromotion.objects.filter(
            ...
            end_date__gte=today,
        ).filter(Q(car=car) | Q(car__isnull=True))

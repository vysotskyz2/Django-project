...
class DealershipRepository:
    def get_active_ids(self) -> list[int]:
        return list(Dealership.objects.filter(is_deleted=False).values_list("id", flat=True))
...
class DealershipInventoryRepository:
    def get_current_stock(self, dealership: Dealership, car) -> int:
        try:
            return DealershipInventory.objects.get(dealership=dealership, car=car).quantity
        except DealershipInventory.DoesNotExist:
            return 0

    def get_all_for_dealership(self, dealership: Dealership) -> QuerySet:
        return DealershipInventory.objects.filter(dealership=dealership).select_related("car")
    ...
    def find_matching_for_car(self, car, price_limit: Decimal) -> list[DealershipInventory]:
        return list(
            DealershipInventory.objects.select_related("dealership", "car").filter(
                car=car,
                quantity__gte=1,
    ...
    def lock_for_dealership_and_car(self, dealership, car) -> DealershipInventory:
        return DealershipInventory.objects.select_for_update().get(dealership=dealership, car=car)
...
class DealershipCarPreferenceRepository:
    def get_preferred_for_dealership(
        ...
        return DealershipCarPreference.objects.filter(
            dealership=dealership, is_preferred=True
        ).select_related("car")

    def get_by_dealership_and_car(
        self, dealership: Dealership, car
    ) -> DealershipCarPreference | None:
        return DealershipCarPreference.objects.filter(dealership=dealership, car=car).first()
...
class DealershipBestSupplierRepository:
    def get_by_dealership_and_car(
        ...
        return (
            DealershipBestSupplier.objects.filter(dealership=dealership, car=car)
            .select_related("supplier")
            .first()
        )

    def get_all_for_dealership(self, dealership: Dealership) -> QuerySet:
        return DealershipBestSupplier.objects.filter(dealership=dealership).select_related(
            "supplier", "car"
        )
    ...
    def upsert(
        ...
        *,
        ...
        with transaction.atomic():
            existing = (
                DealershipBestSupplier.objects.select_for_update()
            ...
            existing = DealershipBestSupplier.objects.select_related("supplier").get(pk=existing.pk)

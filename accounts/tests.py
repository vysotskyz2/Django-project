...
class TestAccountsCeleryTasks:
    def test_send_verification_email(self, monkeypatch):
        from unittest.mock import MagicMock
        from accounts.tasks import send_verification_email

        monkeypatch.setattr(
            'accounts.tasks.EmailService.send_verification',
            MagicMock(),
        )

        send_verification_email('user1', 'u@test.com', 'token-abc')
        from accounts.tasks import EmailService
        EmailService.send_verification.assert_called_once_with(
            'user1', 'u@test.com', 'token-abc',
        )
    ...
    def test_send_password_reset_email(self, monkeypatch):
        ...
        monkeypatch.setattr(
            'accounts.tasks.EmailService.send_password_reset',
            MagicMock(),
        )

        send_password_reset_email('user2', 'u2@test.com', 'token-def')
        from accounts.tasks import EmailService
        EmailService.send_password_reset.assert_called_once_with(
            'user2', 'u2@test.com', 'token-def',
        )

    def test_send_email_change_confirmation(self, monkeypatch):
        from unittest.mock import MagicMock
        from accounts.tasks import send_email_change_confirmation

        monkeypatch.setattr(
            'accounts.tasks.EmailService.send_email_change_confirmation',
            MagicMock(),
        )

        send_email_change_confirmation('user3', 'new@test.com', 'token-ghi')
        from accounts.tasks import EmailService
        EmailService.send_email_change_confirmation.assert_called_once_with(
            'user3', 'new@test.com', 'token-ghi',
        )
